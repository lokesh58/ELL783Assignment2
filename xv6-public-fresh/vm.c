...
int
allocuvm(pde_t *pgdir, uint oldsz, uint newsz, int chgmetadata)
{
  char *mem;
  uint a;
  ...
  for(; a < newsz; a += PGSIZE){
    ...
    if(mem == 0){
      ...
      deallocuvm(pgdir, newsz, oldsz, chgmetadata);
      return 0;
    }
    memset(mem, 0, PGSIZE);
    if(mappages(pgdir, (char*)a, PGSIZE, V2P(mem), PTE_W|PTE_U, chgmetadata) < 0){
